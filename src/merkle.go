...
//Merkle is the container for the tree. It holds a pointer to the root of the tree,
...
func NewTree(cs []Content) (*Merkle, error) {
...
//the tree will be replaced the Merkle completely survives this operation. Returns an error if the
//list of content cs contains no entries.
func (m *Merkle) RebuildTreeWith(cs []Content) error {
	root, leafs, err := buildWithContent(cs)
	if err != nil {
		return err
	}
...
func (m *Merkle) VerifyContent(expectedMerkleRoot []byte, content Content) bool {
